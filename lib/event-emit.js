...
function locationEvent(options) {
  ...
  var bearing = 0;
  if (options.last && options.last.coords) {
    if (options.last.coords[0] !== options.next[0] || options.last.coords[1] !== options.next[1]) {
      bearing = util.bearing(options.last.coords, options.next);
    } else if (options.last.bearing) {
      bearing = options.last.bearing;
    }
  }
  var ev = {
    coords: options.next,
