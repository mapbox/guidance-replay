 ...
 */
function Emitter(geojson, updateInterval, start) {
  if (start !== undefined && start !== 0) {
    geojson = seek(geojson, start);
  }
  ...
  function seek (geojson, start) {
    geojson.geometry.coordinates.splice(0,start);
    geojson.properties.coordinateProperties.speeds.splice(0,start);
    geojson.properties.coordinateProperties.times.splice(0,start);

    // Calibrate the times array to 0ms for the first entry
    geojson.properties.coordinateProperties.times = geojson.properties.coordinateProperties.times.map(function (entry) { return entry - geojson.properties.coordinateProperties.times[0]; });
    return geojson;
  }
}
...
function EmitterPlace(geojson, speedmode) {
  ...
  function constantPoint(i, last) {
    ...
    var percAlong = (i - times[0])/(times[1] - times[0]);
    var dist = util.distance(coords[0], coords[1]) * percAlong;
    var along = util.along(coords[0], coords[1], dist);
    return locationEvent({
      next: along,
      last: last
    });
